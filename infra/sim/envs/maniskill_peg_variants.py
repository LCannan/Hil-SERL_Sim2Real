...
# Copyright 2026 The SERL-Plus-Plus Authors.
#
...
#
# Derived from RLinf `rlinf/envs/maniskill/peg_insertion_side_variants.py`.
# Trimmed to the registration logic SERL++ needs: the RLinf/OpenPI observation
# wrapping, prompt normalisation, and event-tracking helpers are omitted.
#
# The scene geometry and camera placements below are the ground truth for the
# `RLinf/rlt-maniskill-PegInsertionSide-v1-400-succ` dataset.  Changing them
# invalidates any demonstrations converted from it.
...
# SAPIEN sensor names.  These are used verbatim as SERL image keys so that the
# environment and the demo converter cannot drift apart through a rename layer.
MAIN_CAMERA_KEY = "3rd_view_camera"
WRIST_CAMERA_KEY = "wide_hand_camera"

_PEG_VARIANTS_REGISTERED = False


def register_maniskill_peg_variants() -> None:
    """Register the widened PegInsertionSide variants used by `insert_maniskill`.

    Idempotent.  All sapien/mani_skill imports happen inside this function so
    that importing this module stays cheap and does not require the optional
    ManiSkill dependencies -- hosts that only run the MuJoCo tasks never call
    it.
    """
    ...
    try:
        ...
        raise ImportError(
            "The insert_maniskill task requires the optional ManiSkill "
            "dependencies.  Install them with: uv sync --extra maniskill"
        ) from exc
